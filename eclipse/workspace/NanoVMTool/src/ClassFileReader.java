...
public class ClassFileReader implements AccessFlags {
  ...
  private void strClass(DataInput in, ClassInfo classInfo)
    ...
    int superClassNameIndex = 0;
    String superClassName;
    String className = cp.getEntryAtIndex(classNameIndex).getString();
    if(superClassIndex != 0)
    {
    	superClassNameIndex = cp.getEntryAtIndex(superClassIndex).getClassNameIndex();    
    	superClassName = cp.getEntryAtIndex(superClassNameIndex).getString();
    }
    else
    {
    	superClassNameIndex = classNameIndex;
  ...
  private void strMethodAttribute(MethodInfo methodInfo, ConstPool cp, 
	...
	if (name.equals("Exceptions")) {
	  ...
	  for (int i = 0; i < count; i++) {
	    ...
	    Debug.println(Debug.strAttribute, "attribute Exception = " + exceptionName);
	  }
	}
  ...
  private CodeInfo strCode(ConstPool cp, DataInput in) 
    ...
    for (int i = 0; i < exceptionTable.length; i++) {
      ...
      //if (catchTypeIndex != 0) {	// index is null for finally blocks
    	//  int catchTypeNameIndex = cp.getEntryAtIndex(catchTypeIndex).getClassNameIndex();
    	//  catchType = cp.getEntryAtIndex(catchTypeNameIndex).getString();
      //}
      exceptionTable[i] = new ExceptionInfo(startPC, endPC, handlerPC, catchTypeIndex);
    }
    Debug.outdent();
    
    CodeInfo codeInfo = new CodeInfo(maxStack, maxLocals, bytecode, exceptionTable);
