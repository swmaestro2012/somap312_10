...
public class ClassLoader {
	static private Vector<ClassInfo> classes = new Vector();
	...
	public static boolean methodExists(String className, String name,
			String type) {
		// search through all classes
	...
	public static boolean fieldExistsExact(String className, String name,
			String type) {
		// search through all classes
		for (int i = 0; i < classes.size(); i++) {
			ClassInfo classInfo = getClassInfo(i);
	...
	public static boolean fieldExists(String className, String name, String type) {
	...
	public static FieldInfo getFieldInfoExact(String className, String name,
			String type) {
		// search through all classes
		for (int i = 0; i < classes.size(); i++) {
			ClassInfo classInfo = getClassInfo(i);
	...
	public static FieldInfo getFieldInfo(String className, String name,
			String type) {
		while (className != null) {
	...
	public static int getStaticFieldIndex(String className, String name,
			String type) {
		// search through all classes
		for (int i = 0, cnt = 0; i < classes.size(); i++) {
			...
			if (classInfo.getName().equals(className))
				return cnt
						+ classInfo.getFieldIndex(AccessFlags.STATIC, name,
								type);
			else
	...
	// YHC
	public static int totalConstantEntries64() {
		int sum = 0;

		for (int i = 0; i < classes.size(); i++)
			sum += getClassInfo(i).getConstPool().totalConstantEntries64();
	...
	//YHC
	public static long getConstantEntry64(int index) {
		int i = 0;

		// search through all classes
		while (index >= getClassInfo(i).getConstPool().totalConstantEntries64())
			index -= getClassInfo(i++).getConstPool().totalConstantEntries64();

		return getClassInfo(i).getConstPool().getConstantEntry64(index);
	}
	...
	static public void load(String name) {
		...
		try {
			...
			StringTokenizer pathTokenizer = new StringTokenizer(classPath,
					File.pathSeparator);
			...
			if (is == null)
				throw new FileNotFoundException("Unable to find class file "
						+ name + ".class");
