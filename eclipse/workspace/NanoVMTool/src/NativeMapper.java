...
public class NativeMapper {
			...
			while ((line = reader.readLine()) != null) {
				...
				while (st.hasMoreTokens() && !skipRest) {
					...
					if (token.charAt(0) == '#')
						...
						if (token_cnt == 0)
							...
							System.out.println("Ignoring superfluous data: "
									+ token);
						}
				...
				if (token_cnt > 0) {
					...
					// method entry
						...
						nativeMethod.name = value.substring(0,
								value.indexOf(':'));
						nativeMethod.type = value.substring(
								value.indexOf(':') + 1, value.length());
						nativeMethod.id = Integer.parseInt(id)
								+ (fullClassId << 8);
					...
					// field entry
						...
						nativeField.name = value.substring(0,
								value.indexOf(':'));
						nativeField.type = value.substring(
								value.indexOf(':') + 1, value.length());
						nativeField.id = Integer.parseInt(id)
								+ (fullClassId << 8);
						...
						System.out.println("Unknown entry in native file: "
								+ name + " + " + token_cnt + " parms");
						System.exit(-1);
					}
				}
			}
	...
	public static boolean classIsNative(String className)
	{
		for (int i = 0; i < nativeMethods.size(); i++) {
			NativeMethod nativeMethod = (NativeMethod) nativeMethods
					.elementAt(i);
	...
	public static boolean methodIsNative(String className, String name,String type) {
	...
	public static int getMethodId(String className, String name, String type) {
	...
	public static int getFieldId(String className, String name, String type) {
