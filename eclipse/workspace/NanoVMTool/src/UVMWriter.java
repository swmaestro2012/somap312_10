...
public class UVMWriter {
  ...
  void write64(long val) throws ConvertException {
	  	write8((int)(val&0xff));
	    write8((int)((val>>8)&0xff));
	    write8((int)((val>>16)&0xff));
	    write8((int)((val>>24)&0xff));
	    write8((int)((val>>32)&0xff));
	    write8((int)((val>>40)&0xff));
	    write8((int)((val>>48)&0xff));
	    write8((int)((val>>56)&0xff));	    
  }
  ...
  void writeHeader() throws ConvertException {
    ...
    int offset = 17;    // header size: 17 bytes	//yhc
    ...
    write16(offset);
    System.out.println("constant data offset = "+offset);
    
    // offset to string data
    offset += 4 * ClassLoader.totalConstantEntries(); // constant value size: 4bytes
    write16(offset);
    System.out.println("string data offset = "+offset);
    
    // offset to constant64 data
    offset += 2 * ClassLoader.totalStrings(); // string indices
    offset += ClassLoader.totalStringSize();  // string data
    write16(offset);
    System.out.println("constant64 data offset = "+offset);

    // offset to method data    
    offset += 8 * ClassLoader.totalConstantEntries64(); // constant value size: 8bytes //YHC
    write16(offset);
    System.out.println("method data offset = "+offset);
  ...
  void writeClassHeaders() throws ConvertException {
    for(int i=0;i<ClassLoader.totalClasses();i++) {
      ...
      int superclassindex = classInfo.getSuperClassIndex();
      int nonstaticfields = classInfo.nonStaticFields();
      write8(superclassindex);
      write8(nonstaticfields);
    }
  }
  ...
  // write all 64bit constant values
  void writeConstantEntries64() throws ConvertException {
    System.out.println("Writing " + ClassLoader.totalConstantEntries64() + " constant entries");
    for(int i=0;i<ClassLoader.totalConstantEntries64();i++) {
      System.out.println("  entry64[" + (i) + "] = 0x" + Long.toHexString(ClassLoader.getConstantEntry64(i)));
      write64(ClassLoader.getConstantEntry64(i));
    }
  }
  ...
  void writeMethods() throws ConvertException {
    ...
    for(int i=0;i<ClassLoader.totalMethods();i++) {
      ...
      System.out.println("write method[" + i + "] : " + ClassLoader.getClassInfo(ClassLoader.getClassIndex(i)).getName() + "." + methodInfo.getName());
      // offset from this header to bytecode (this header is 8 bytes
      // in size)
      write16((ClassLoader.totalMethods()-i) * 10 + codeOffset);      // exception_index
      write16((ClassLoader.totalMethods()-i) * 10 + codeOffset + (methodInfo.getCodeInfo().getExceptionTable().length * 8));      // code_index 
      write16((ClassLoader.getClassIndex(i) << 8) + MethodIdTable.getEntry(i));                        // id
      method_flag = methodInfo.getName().equals("<clinit>")?(byte)1:0;
      if ((methodInfo.getAccessFlags() & AccessFlags.SYNCHRONIZED) == AccessFlags.SYNCHRONIZED)
    	  method_flag |= 2;		
      write8(method_flag);       // flags
      write8(methodInfo.getArgs());                              // args
      max_locals = methodInfo.getCodeInfo().getMaxLocals();
      write8(max_locals);           // max_locals
      write8(methodInfo.getCodeInfo().getMaxStack());            // max_stack
      
      /*temp_length = methodInfo.getCodeInfo().getExceptionTable().length;
      if(temp_length > 0)
      {
	      exceptionTable = methodInfo.getCodeInfo().getExceptionTable();
	      if(NativeMapper.classIsNative(exceptionTable[0].catchType)) {
	    	  System.out.println(exceptionTable[0].catchType + " is native");
	      }
      } */
      codeOffset += methodInfo.getCodeInfo().getExceptionTable().length * 8;	// YHC - Exception Table StartPC, EndPC, HandlePC, catchClassType : total 8byte
    ...
    for(int i=0;i<ClassLoader.totalMethods();i++) {
      ...
      MethodInfo methodInfo = ClassLoader.getMethod(i);
      ExceptionInfo[] exceptionTable = methodInfo.getCodeInfo().getExceptionTable();
      
      // write exception table
      System.out.println("Write exception table");
      for(int j=0;j<exceptionTable.length;j++)
      {
    	  System.out.print("exception table[" + j + "] = startPC: " + exceptionTable[j].startPC + ", endPC: " + exceptionTable[j].endPC + ", handlerPC: " + exceptionTable[j].handlerPC);
    	  write16(exceptionTable[j].startPC);
    	  write16(exceptionTable[j].endPC);
    	  write16(exceptionTable[j].handlerPC);
    	  if(exceptionTable[j].catchType != 0)
    	  {
    		  System.out.print(", catch type #" + exceptionTable[j].catchType);
    		  write16(classInfo.getConstPool().constantRelocate(exceptionTable[j].catchType));
    	  }
    	  else
    	  {
    		  System.out.println(", catch type #" + exceptionTable[j].catchType + " -> is for finally");
    		  write16(0);
    	  }
    	  //if(NativeMapper.classIsNative(exceptionTable[i].catchType)) {
    		  //write16(exceptionTable[i].catchType);
    	  //}
      }
      
      
      System.out.println("#" + i + " Method Converting " + 
			 classInfo.getName() + "." +
			 methodInfo.getName() + ":" +
  ...
  public UVMWriter(boolean writeHeader) {
    ...
    try {
      ...
      writeConstantEntries64();  // write all 64-bit constants // YHC
