...
public abstract class Uploader {
	...
	public abstract void doUpload(String device, int target, int speed,
			byte[] file, int length);

	public static void upload(String device, int target, int speed,
			byte[] file, int length) {
	...
	public static void main(String[] args) {
		...
		if (args[1].equalsIgnoreCase("asuro"))
			upload(args[0], Config.TARGET_NVC1_ASURO, 2400, buffer,
					buffer.length);
		else
			upload(args[0], Config.TARGET_NVC1_UART, Integer.parseInt(args[1]),
