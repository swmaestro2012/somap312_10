...
//static nvm_stack_t *j_sp;        // the current stack pointer
...
void stack_save_sp(void) {
  ...
  sp_saved = current_task->j_sp;  // save stack pointer
}
...
void stack_init(u08_t static_fields) {
  ...
  current_task->j_stack = (nvm_stack_t*)heap_get_base();
...
nvm_long_t stack_pop_long(void) {
	nvm_long_t t = nvm_stack2long((nvm_stack_t*)current_task->j_sp);
	current_task->j_sp--;
	current_task->j_sp--;  
	return t;
}
...
nvm_stack_t stack_peek(u08_t index) {
  return current_task->j_sp[-index];
}
...
{
  return nvm_stack2float(*(current_task->j_sp--));
}

nvm_float_t stack_peek_float(u08_t index)
{
  return nvm_stack2float(current_task->j_sp[-index]);
}
#endif

nvm_long_t stack_peek_long(u08_t index) {
  return nvm_stack2long(&current_task->j_sp[-index]);
}
...
{
  return nvm_stack2double(&current_task->j_sp[-index]);
}
...
nvm_stack_t stack_get_static(u16_t index) {
  return current_task->j_stack[index];
}

void stack_set_static(u16_t index, nvm_stack_t value) {
  current_task->j_stack[index] = value;
}
...
bool_t stack_is_empty(void) {
  ...
  return(current_task->j_sp == current_task->j_stackbase);
}
...
bool_t stack_heap_id_in_use(heap_id_t id) {
  ...
  // to search the stack
  for(i=0;i<current_task->j_sp-current_task->j_stack+1;i++) {
//    DEBUGF("Stack %d == "DBG16"\n", i, stack[i]);
    if(current_task->j_stack[i] == id16) return TRUE;
  }
