...
void native_ctbot_motor_invoke(u08_t mref) {
  ...
  if(mref == NATIVE_METHOD_setLeft) { // set Speed
		nvm_int_t channel, value;
		value = stack_pop_int();
		channel = stack_pop_int();
		set_motor_speed(channel, value);
  }

  // JAVA: void setRight(int val)
  else if(mref == NATIVE_METHOD_setRight) { // stop
		set_motor_speed(0,0);
		set_motor_speed(1,0);
  }
