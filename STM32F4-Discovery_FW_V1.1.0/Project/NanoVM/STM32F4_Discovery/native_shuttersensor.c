...
void native_ctbot_shuttersensor_init(void) {
//  deactivate_output_bit(IO_SENS_DOOR);
}

// the ctbot class
void native_ctbot_shuttersensor_invoke(u08_t mref) {

  // JAVA: boolean getState()
  if(mref == NATIVE_METHOD_getState) {
//    nvm_int_t val = get_input_bit(IO_SENS_DOOR)?1:0;
		nvm_int_t val = 0;
    stack_push(val);
  }
  ...
  // JAVA: void setEnabled()
    ...
    if (stack_pop_int())
      shift_reg_ena &= ~(uint8_t)ENA_KLAPPLED;
    else
      shift_reg_ena |= ENA_KLAPPLED;
    shift_data(shift_reg_ena, SHIFT_REGISTER_ENA);
  }

  // JAVA: boolean getEnabled()
  else if(mref == NATIVE_METHOD_getEnabled) {
    nvm_int_t val = (shift_reg_ena & ENA_KLAPPLED)?1:0;
    stack_push(val);
  }
