...
u08_t nvmfile_constant64_count;
...
nvm_ref64_t nvmfile_read64(void *addr) {
	nvm_ref64_t ret;
	addr = NVMFILE_ADDR(addr);  // remove marker (if present)
	memcpy((u08_t*)&ret, addr, sizeof(ret));	
	return ret;
}
...
bool_t nvmfile_init(void) {
  ...
  nvmfile_constant_count = t/4;
	...
	t = nvmfile_read16(&((nvm_header_t*)nvmfile)->method_offset);
  t -= nvmfile_read16(&((nvm_header_t*)nvmfile)->constant64_offset);
	nvmfile_constant64_count = t/8;
  return TRUE;
}
...
nvm_ref64_t nvmfile_get_constant64(u08_t index) {
	nvm_ref64_t ret;
	u16_t addr = nvmfile_read16(&((nvm_header_t*)nvmfile)->constant64_offset);
	ret = nvmfile_read64(nvmfile+addr+8*index);
	return ret;
}
