...
void native_ltoa(char *str, nvm_long_t val) {
	nvm_ulong_t uval = val;
  nvm_long_t m;
  nvm_long_t div = 1000000000000000000;  // max num to be output 
  bool_t printed = FALSE;
...
#endif    
  } else if(mref == NATIVE_METHOD_PRINT_LONG)	{ //YHC
		native_ltoa((char*)tmp, stack_pop_long());
    native_print(tmp, FALSE);
	}	else
    ...
    *(char*)stack_pop_addr() = 0;
    ...
    stack_pop(); stack_pop(); 
	    ...
	    (mref == NATIVE_METHOD_APPEND_CHR)||
			...
			(mref == NATIVE_METHOD_APPEND_LONG)|| // YHC
			(mref == NATIVE_METHOD_APPEND_DOUBLE)	// YHC
	) {
    char *src0, *src1, *dst;
    char tmp[25];
    u16_t len;
...
#endif
      } else if(mref == NATIVE_METHOD_APPEND_LONG) {
				native_ltoa(tmp, stack_peek_long(0));
				sidx = 2;
			} else if(mref == NATIVE_METHOD_APPEND_DOUBLE) {
				native_dtoa(tmp, stack_peek_double(0));
				sidx = 2;
			} else {
